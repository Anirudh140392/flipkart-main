...
const Header = () => {
    ...
    const { dateRange, formatDate, campaignSetter } = useContext(overviewContext) || { dateRange: [{ startDate: new Date(), endDate: new Date() }] };

    const [showSelectedOperator, setShowSelectedOperator] = useState(operatorType ? operatorType : OPERATOR.FLIPKART);
    ...
    /*useEffect(() => {
        if (showSelectedOperator === 'Zepto') {
            if (window.location.pathname === '/') {
                navigate(`/smart-control?operator=${showSelectedOperator}`)
            }
        }
    }, [showSelectedOperator])*/
    ...
    const onHamburgerClick = () => {
        let sideNavMain = document.getElementsByClassName('left-navbar-main-con')[0];
        ...
        sideNavMain.classList.value = sideNavMain.classList.value === 'left-navbar-main-con' ? 'left-navbar-main-con hide-sidenavbar' : 'left-navbar-main-con';
        mainContainer.classList.value = mainContainer.classList.value === 'main-con' ? 'main-con hide-sidenavbar' : 'main-con';
        headerContainer.classList.value = headerContainer.classList.value === 'header-main-con' ? 'header-main-con hide-sidenavbar' : 'header-main-con';
            ...
            <div className="header-main-con">
                ...
                <div className="d-flex actions-con">
                    ...
                    <Dropdown className="operator-selected-tab">
                        <Dropdown.Toggle variant="white" id="dropdown-basic" disabled>
                            {showSelectedOperator}
                        </Dropdown.Toggle>
...
const OperatorList = (props) => {
    const { showSelectedOperator,
        setShowSelectedOperator,
        selectedOperator } = props;
    return (
        <Dropdown.Item onClick={() => setShowSelectedOperator(selectedOperator)}>{selectedOperator}</Dropdown.Item>
    )
}
